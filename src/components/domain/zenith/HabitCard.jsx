...
export default function HabitCard({
  ...
  return (
    ...
    >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
          ...
          <button
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              cursor: (checking || isSubmitting) ? 'default' : 'pointer', transition: 'all 0.2s',
              outline: 'none', padding: 0, opacity: (checking || isSubmitting) ? 0.5 : 1
            }}
            onClick={handleCheck}
          >
            {isCompleted && (
      ...
      {/* Compact Impact Insight Line */}
      {impactInsight && (
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          marginTop: '8px', paddingTop: '6px', borderTop: '1px dashed #1A1A24',
          fontSize: '9px', fontWeight: 700
        }}>
          <span style={{
            color: (impactInsight.focusDifference || 0) > 0
              ? 'var(--color-success)'
              : (impactInsight.focusDifference || 0) < 0
                ? 'var(--color-warning)'
                : 'var(--text-muted)'
          }}>
            ⚡ {impactInsight.explanation}
          </span>
          <span style={{ fontSize: '8px', color: '#444455' }}>
            {impactInsight.disclaimer}
          </span>
        </div>
      )}
    </div>
