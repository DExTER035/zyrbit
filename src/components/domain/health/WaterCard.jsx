...
export default function WaterCard({ todayWater = 0, dynamicTarget = 3000, onLogWater, onDeleteWater, waterLogs = [], isSubmitting = false }) {
  ...
  return (
    <div style={{
      ...
      {/* ── Recent Water Logs List with Delete ── */}
      {onDeleteWater && waterLogs.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '4px', borderTop: `1px solid ${C.border}`, paddingTop: '8px' }}>
          <span style={{ fontSize: '10px', color: C.muted, fontWeight: 700 }}>Today's Water Logs ({waterLogs.length})</span>
          {waterLogs.slice(-3).map(log => (
