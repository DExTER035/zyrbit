...
import { ChevronLeft, CheckCircle2, Timer, Plus, Lock, Link, Trash2, Play } from 'lucide-react';
import {
  C,
  daysUntil,
  fmtHours,
...
export default function ProjectDetailView({
  ...
  return (
    <div className="page-enter" style={{ background: C.bg, minHeight: '100vh', color: C.text }}>
      ...
      {/* Tasks */}
      <div style={{ padding: '0 20px 16px' }}>
        <SectionHeader title={`Execution Tasks (${todoList.length} open)`} action={() => setModalTask(true)} actionLabel="Add" actionIcon={Plus} />
        {ptasks.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {ptasks.map(t => {
              ...
              const possiblePrereqs = ptasks.filter(other => other.id !== t.id && other.status !== 'done');
              ...
              return (
                <div
                  ...
                  style={{
                    background: C.card,
                    border: `1px solid ${state === 'BLOCKED' ? C.warn + '40' : state === 'AVAILABLE' ? '#14B8A640' : C.border}`,
                    borderRadius: '14px',
                    padding: '14px',
                    display: 'flex',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, minWidth: 0 }}>
                      <button
                        ...
                        disabled={isDone}
                        style={{
                          background: isDone ? C.goal : 'none',
                          border: `1.5px solid ${isDone ? C.goal : C.muted}`,
                          width: '20px',
                          height: '20px',
                          borderRadius: '50%',
                      ...
                      <span style={{ fontSize: '14px', fontWeight: 700, color: C.text, textDecoration: isDone ? 'line-through' : 'none' }}>
                        {t.name}
                      </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <Pill
                        label={state}
                        color={state === 'DONE' ? C.muted : state === 'AVAILABLE' ? '#14B8A6' : C.warn}
                      />
                      {deleteTask && (
                        <button
                          type="button"
                  ...
                  {!isDone && (
                    <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: '8px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '6px' }}>
                        <span style={{ fontSize: '10px', color: C.muted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                          Prerequisites ({taskDeps.length})
                        </span>
                        {possiblePrereqs.length > 0 && (
                          <select
                            onChange={e => {
                              if (e.target.value) {
                                addDependency(t.id, e.target.value);
                                e.target.value = '';
                              }
                            }}
                            style={{
                      ...
                      {/* Active Prerequisite List */}
                      {taskDeps.map(dep => {
                        const prereqTask = tasks.find(x => x.id === dep.depends_on_task_id);
                        return (
                          <div key={dep.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '4px', background: C.surface, padding: '4px 8px', borderRadius: '6px', fontSize: '11px', minWidth: 0 }}>
                            <span style={{ color: prereqTask?.status === 'done' ? C.goal : C.sub, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 'calc(100% - 55px)' }}>
                              {prereqTask?.status === 'done' ? '✓' : '🔒'} {prereqTask?.name || 'Unknown task'}
                            </span>
                            <button
                              type="button"
                              onClick={() => removeDependency(dep.id)}
                              style={{ background: 'none', border: 'none', color: C.muted, cursor: 'pointer', fontSize: '10px', flexShrink: 0, padding: '2px 4px' }}
                            >
                              Remove
                            </button>
                          </div>
